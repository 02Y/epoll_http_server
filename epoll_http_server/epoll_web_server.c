...
struct _ConnectStat {
	...
	char name[64];
	char  age[64];
	struct epoll_event _ev;    //当前文件句柄对应的epool_event
...
int startup(char* _ip, int _port)  //创建一个套接字，绑定，检测服务器
{
	...
	int opt = 1;
	setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));    //设置地址重用
...
int main(int argc, char *argv[])
{
	...
	//1.创建epoll    
	epfd = epoll_create(256);    //可处理的最大句柄数256个（并没有限制）
	if (epfd < 0)
	{
		perror("epoll_create");
		exit(5);
	...
	struct epoll_event revs[64];   //把发生的事件复制到数组中
			...
			for (i = 0; i < num; i++)
			{
				...
				if (rsock == listen_sock && (revs[i].events) && EPOLLIN) //如果是初始的 就接受，建立链接
				{
					...
					if (new_fd > 0)
					{
						...
						//sleep(1000);
						connect_handle(new_fd);     //新的fd加入托管
					}
				}
...
<h2>欢迎学霸同学&nbsp;%s &nbsp;,你的芳龄是&nbsp;%s！</h2><br/><br/>\n\
	...
	stat->_ev.events = EPOLLIN;      //等待客户端再次发送请求
	//stat->_ev.data.ptr = stat;
	epoll_ctl(epfd, EPOLL_CTL_MOD, stat->fd, &stat->_ev);
}
